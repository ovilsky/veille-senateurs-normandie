...
def build_senateur_id_index(senateurs_rows):
    ...
    Piège découvert au premier run réel : senateurs.csv contient 1948
    lignes (tous les sénateurs de l'histoire disponible, actifs et
    inactifs — colonne "active"), pas seulement les 348 en exercice. Un
    matching par nom seul risquerait donc d'attraper un homonyme d'une
    autre époque plutôt que le sénateur actuel. On filtre en priorité sur
    active == "True", avec repli sur le département si l'ambiguïté
    ...
    col_nom = find_column(fieldnames, ["last", "name"], ["nom"])
    col_prenom = find_column(fieldnames, ["first", "name"], ["prenom"])
    col_dept = find_column(fieldnames, ["department", "label"], ["dept"], ["circonscription"])
    col_id = find_column(fieldnames, ["slug"], ["id"])
    col_active = find_column(fieldnames, ["active"])
    ...
    for s in SENATEURS:
        ...
        # Priorité aux sénateurs actifs : évite d'attraper un homonyme
        # d'une autre époque parmi les 1948 lignes historiques.
        if col_active and len(candidates) > 1:
            active_candidates = [c for c in candidates if c.get(col_active, "").strip().lower() == "true"]
...
def build_scrutins_groupes(our_rows, votes_solennels, reverse_id_index, slug_to_senateur,
                            id_to_groupe, group_tally):
    """
    Regroupe les lignes (scrutin_id, senateur_id, position) — une par
    sénateur suivi et par scrutin — en une liste d'enregistrements PAR
    SCRUTIN, chacun portant ses métadonnées globales (résultat du vote)
    et la liste des positions individuelles de nos sénateurs suivis.

    C'est cette structure (et non plus une ligne à plat par paire
    sénateur/scrutin) qu'attend le dashboard pour afficher une carte par
    scrutin, à l'image de la page de référence veille-deputes-normandie.
    Séparée de main() pour être testable hors-ligne sur des données
    synthétiques, indépendamment du téléchargement réel des CSV.
    """
    scrutins_par_id = {}
    ...
    for scrutin_id, senateur_id, position in our_rows:
        ...
        if scrutin_id not in scrutins_par_id:
            votants = to_int(scrutin.get("votants"))
            suffrages_exprimes = to_int(scrutin.get("suffrages_exprimes"))
            pour = to_int(scrutin.get("pour"))
            contre = to_int(scrutin.get("contre"))
            # "abstentions" n'existe pas comme colonne dédiée dans
            # scrutins_senat.csv : on la déduit quand les nombres
            # nécessaires sont disponibles (sinon on laisse None plutôt
            # que d'afficher un chiffre faux).
            abstentions = None
            if suffrages_exprimes is not None and pour is not None and contre is not None:
                abstentions = suffrages_exprimes - pour - contre
            elif votants is not None and suffrages_exprimes is not None:
                abstentions = votants - suffrages_exprimes

            scrutins_par_id[scrutin_id] = {
                "type": "scrutin",
                ...
                "dossier_ref": scrutin.get("dossier_ref"),
                "votants": votants,
        ...
        scrutins_par_id[scrutin_id]["positions"].append({
            "senateur_slug": slug,
    ...
    results = list(scrutins_par_id.values())
    for scrutin in results:
        scrutin["positions"].sort(key=lambda p: normalize(p["senateur_nom"]))
...
def main():
    ...
    # "group_label" donne le nom complet (ex. "Les Républicains"), cohérent
    # avec le style de senateurs_config.py — utilisé pour l'affichage.
    col_groupe = find_column(senateurs_fieldnames, ["group", "label"], ["groupe"])
    ...
    col_senateur_id = find_column(
        fieldnames,
        ["senator", "slug"], ["senateur", "slug"],  # nom réel observé (anglais) + repli français
        ["senator", "id"], ["senateur", "id"], ["deput", "id"], ["parlementaire", "id"],
    )
    ...
    # Regroupement par scrutin : un enregistrement par scrutin, contenant
    # ses métadonnées (résultat global) et la liste des positions
    # individuelles de nos sénateurs suivis — plutôt qu'une ligne à plat
    # par paire (sénateur, scrutin). C'est ce que consomme le dashboard
    # pour afficher une carte par scrutin (cf. page de référence
    # veille-deputes-normandie).
    results = build_scrutins_groupes(
        our_rows, votes_solennels, reverse_id_index, slug_to_senateur,
        id_to_groupe, group_tally,
    )

    total_positions = sum(len(r["positions"]) for r in results)
    ...
    print(
        f"\n{len(results)} scrutin(s) solennel(s) concernant nos sénateurs, "
        f"{total_positions} position(s) individuelle(s) au total.",
        file=sys.stderr,
    )
