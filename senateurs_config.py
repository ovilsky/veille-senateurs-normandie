...
for _s in SENATEURS:
    ...
    # Prénom / nom de famille séparés (dérivés automatiquement du champ
    # "nom" : premier mot = prénom, reste = nom de famille — fonctionne
    # pour tous nos cas, y compris les noms composés comme
    # "Sonia de La Provôté" ou "Catherine Morin-Desailly"). Utilisé pour
    # matcher les colonnes Nom/Prénom du fichier Open Data du Sénat
    # (data.senat.fr) dans fetch_activite_senateurs.py.
    _parts = _s["nom"].split(" ", 1)
    _s["prenom"] = _parts[0]
    _s["nom_famille"] = _parts[1] if len(_parts) > 1 else ""
    # Photo officielle senat.fr : format vérifié par l'utilisateur le
    # 19/09/2026 sur une fiche réelle (https://www.senat.fr/senimg/
    # allizard_pascal14133k_carre.jpg) — dérivé du slug pour les 17.
    # Peut casser après le renouvellement du 27/09/2026 si le slug change
    # pour les départements en série 2 ; le dashboard a un repli visuel
    # (avatar à initiales) si l'image ne charge pas.
    _s["photo_url"] = f"https://www.senat.fr/senimg/{_s['slug']}_carre.jpg"
